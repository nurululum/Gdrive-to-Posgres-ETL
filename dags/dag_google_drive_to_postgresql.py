...
def __authenticate():
    ...
    token_path = Variable.get(key="dag_google_drive_to_postgresql_prod")
...
def _check_file_exists(ti):
    ...
    try:
        ...
        dataset_files = (
            service.files()
            .list(q=f"'{dataset_folder_id}' in parents", pageSize=20, fileds="nextPageToken, files(id, name)")
            .execute()
        ).get("files", [])
...
def _get_data(ti):
    ...
    # pull xcom
    dataset_files = ti.xcom_pull(key="dataset_files", task_ids="check_file_exists")
    ...
    for file in dataset_files:
        ...
        with open(input_file, 'r', newline='') as csvfile:
            ...
            list_processed_name = []
            with open(output_file, 'w', newline='') as output_csvfiles:
                # Create CSV reader and writer objects
                reader = csv.DictReader(csvfile)
                fieldnames = reader.fieldnames + ['id']
                writer = csv.DictWriter(output_csvfiles, fieldnames=fieldnames)
...
def _ingest_to_postgres(ti):
    ...
    # CSV file path
    # pull xcom
    list_processed_name = ti.xcom_pull(key="list_processed_name", task_ids="get_data")
...
with DAG(
    dag_id='dag_google_drive_to_postgresql',
    ...
    start >> check_file_exists >> [get_data, end]
